...
public class MainActivity extends BaseActivity {
    ...
    private TextView mChechUpAll;
    private TextView mTvCheckInfo;
    private RecyclerView.LayoutManager layoutManager;
    private List<ActivityManager.RunningAppProcessInfo> taskList;
    ...
    public void dowloadApplication(String url){
        ...
        OkHttpUtils.get()
                .url(url)
                .build()
                .execute(new FileCallBack(getExternalCacheDir().getAbsolutePath(),apkName)//
                {
                    @Override
...
*/
                    ...
                    public void onResponse(final File response, int id) {
                        log4jUtils.debug("**************MainActivity 安装包下载完成 ****");
    ...
    public void install(Context context, final String apkName, String apkPath){
        ...
        InstallHelper.execute(MainActivity.this,apkPath, log4jUtils, new InstallHelper.IInstallHelper() {
            ...
            public void onError(final Exception e) {
                log4jUtils.error(TAG +" 安装失败："+ apkName+ e.getMessage());
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
